					...
					for(c=0;c<options.count;c++){
						if(a===this.indexArr[c]){
							var zindex = a>CenIdxArr?a-t:a,
								...
								width = options.width-tmp*options.imgwidth,
								height = options.height-options.imgheight*tmp,
								top = options.imgtop*tmp,
								left = Math.round(options.offsetX*tmp)-options.imgleft*tmp;
							if(a<CenIdxArr){
								t2-=1;
								left = left * -1;
							};
