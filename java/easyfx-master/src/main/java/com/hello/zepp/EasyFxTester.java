...
/**
 * java -cp easyfx-master.jar com.hello.zepp.EasyFxTester
...
//        TaylorSeries.setArithmetic(MyTaylorSeries.class);//自定义算法
        easyFX.setParallelNu(7).setOps(op1, op2,op3).execute();
        double multiply = Math.sqrt(op1.val + op2.val*(op1.val+1)/2+op3.val);
